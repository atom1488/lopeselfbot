...
export const name = 'spamping';
export const description = `spam les pings | ${config.prefix}spamping <id>`
export function execute(self, message, args) {
    if(args == '') return message.edit(`Veuillez mettre l'ID d'un utilisateur | ${config.prefix}spamping <id>`)
    message.delete();
    var id = args[0]
    ...
    function spamMessage() {
        message.channel.send(`<@${id}>, ${randomText[(Math.floor(Math.random() * randomText.length))]} `).catch((err) => {
